...
<html>
    <head>
        ...
        <title><fmt:message key="${specialty.name}" bundle="${application}"/></title>
    ...
    <body>
        ...
        <fmt:message var="back_message" key="back.specialty.message" bundle="${application}"/>
        ...
        <h3 class="title"><fmt:message key="${specialty.name}" bundle="${application}"/></h3>
        <div class="table-container">
            <div class="table-form">
                <fmt:message key="admission.places.amount" bundle="${application}"/>: ${specialty.maxStudentAmount}<br>
                <hr>
                <fmt:message key="requirements" bundle="${application}"/>:<br>
                <div class="reqs">
                    <c:forEach items="${specialty.requirements}" var="requirement">
                        <fmt:message key="${requirement.subject}" bundle="${application}"/>: ${requirement.mark}<br>
                    </c:forEach>
                </div>
            </div>
            <div class="form-button" onclick="popup_function()">
                <c:if test="${sessionScope.exception.message == 'Admission is closed!'}">
                    <span class="popup_text" id="my_popup">
                        <fmt:message key="admission.closed.exception" bundle="${application}"/>
                        <c:remove var="exception" scope="session" />
                    </span>
                </c:if>
                <c:if test="${sessionScope.exception.message == 'User already has application!'}">
                    <span class="popup_text" id="my_popup">
                        <fmt:message key="application.exists.exception" bundle="${application}"/>
                        <c:remove var="exception" scope="session" />
                    </span>
                </c:if>
                <c:if test="${sessionScope.exception.message == 'Insufficient marks for specialty admission!'}">
                    <span class="popup_text" id="my_popup">
                        <fmt:message key="insufficient.marks.exception" bundle="${application}"/>
                        <c:remove var="exception" scope="session" />
                    </span>
                </c:if>
                <a class="btnSubmit" href="${pageContext.request.contextPath}/api/student/specialty/apply?${specialty.name}&locale=${sessionScope.locale}">
                    <fmt:message key="apply" bundle="${application}"/>
                </a>
            </div>
        </div>
    </body>
